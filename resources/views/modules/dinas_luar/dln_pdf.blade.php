...
<body>
  <table class="no-border" style="width:100%;">
    <tr>
      ...
      <td class="header">
        ...
        <div class="sub">Form Pengajuan Dana Dinas Luar Negeri</div>
      </td>
  ...
  <table class="info-table">
    ...
    <tr>
      <td>Negara</td><td>:</td><td>{{ $row->country ?? '-' }}</td>
      <td>Kota</td><td>:</td><td>{{ $row->city ?? '-' }}</td>
    </tr>
    <tr>
      <td>No Paspor</td><td>:</td><td>{{ $row->passport_no ?? '-' }}</td>
      <td>Paspor Exp</td><td>:</td><td>{{ $row->passport_expiry ? format_date_id($row->passport_expiry) : '-' }}</td>
    </tr>
    <tr>
      <td>Currency</td><td>:</td><td>{{ $row->currency ?? '-' }}</td>
