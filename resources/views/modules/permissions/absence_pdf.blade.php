  ...
  <table class="table header-table">
    <tr>
      ...
      <td class="header-right">
        <div class="small">Nomor: IZIN/{{ (int) $item->id }}/{{ strtoupper($company->company_code ?? '') }}/{{ date('Y') }}</div>
        <div class="small">Tanggal Cetak: {{ date('d/m/Y') }}</div>
      </td>
    </tr>
  </table>

  <div class="title">SURAT IZIN TIDAK MASUK KERJA</div>
  <div class="subtitle">Izin / Cuti / Sakit</div>
  ...
  <div class="section box">
    <table class="table">
      ...
      <tr>
        ...
        <td style="text-align:right;">{{ $days }} hari</td>
      </tr>
      <tr>
        <td class="label">Periode</td>
        <td colspan="3">{{ format_date_id($item->date_start) }} - {{ format_date_id($item->date_end) }}</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="3">{{ !empty($item->doctor_note_path) ? 'Surat Dokter (Ada)' : 'Tidak ada lampiran' }}</td>
      </tr>
    </table>
  </div>

  <div class="section">
    <div class="small muted">Dengan ini menyatakan pengajuan izin tidak masuk kerja sesuai data di atas.</div>
  </div>
