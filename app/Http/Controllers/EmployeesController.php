...
{
    private function buildEmployeesExportQuery(Request $request, array $user, int $companyId)
    {
        $allowedViewModes = ['active_all', 'active_tetap', 'active_kontrak', 'active_percobaan', 'active_harian', 'active_freelance', 'active_komisaris', 'archive', 'mutasi'];
        ...
        $viewMode = in_array($requestedView, $allowedViewModes, true)
            ? $requestedView
            : (in_array($sessionView, $allowedViewModes, true) ? $sessionView : 'active_tetap');
        ...
        if ($viewMode === 'mutasi') {
            ...
            if ($viewMode === 'archive') {
                ...
                if ($viewMode === 'active_tetap') {
                    ...
                    $employeesQuery->where(function ($q2) {
                        $q2->whereRaw('LOWER(COALESCE(employment_status, "")) LIKE ?', ['%percobaan%'])
                ...
                } elseif ($viewMode === 'active_freelance') {
                    $employeesQuery->where(function ($q2) {
                        $q2->whereRaw('LOWER(TRIM(COALESCE(employment_status, ""))) = ?', ['freelance'])
                            ->orWhereRaw('LOWER(TRIM(COALESCE(employment_status, ""))) = ?', ['frelance']);
                    });
        ...
        return $employeesQuery->orderBy('name');
    }
        ...
        if ($viewMode === 'mutasi') {
            ...
            if ($viewMode === 'archive') {
                ...
                if ($viewMode === 'active_tetap') {
                    ...
                    $employeesQuery->where(function ($q) {
                        $q->whereRaw('LOWER(TRIM(COALESCE(employment_status, ""))) = ?', ['freelance'])
                            ->orWhereRaw('LOWER(TRIM(COALESCE(employment_status, ""))) = ?', ['frelance']);
                    });
                } elseif ($viewMode === 'active_komisaris') {
                    $employeesQuery->whereRaw('LOWER(TRIM(COALESCE(employment_status, ""))) = ?', ['komisaris']);
    ...
    public function form(Request $request)
    {
        ...
        $canEmployeesBase = rbac_key_allowed($role, 'employees');
        $canProfile = rbac_key_allowed($role, 'employees_form_profile') || $canEmployeesBase;
        $canPayroll = rbac_key_allowed($role, 'employees_form_payroll') || $canEmployeesBase;
        $companyId = current_company_id();
        $companies = Company::orderBy('id')->get();
        ...
        if ($request->isMethod('post')) {
            ...
            $payrollData = [
                ...
                'absence_mode' => $request->input('absence_mode', 'auto'),
                'manual_present_days' => $request->input('manual_present_days', 0),
    ...
    public function allInOvertime(Request $request)
    {
        $user = current_user();
        if (current_user_has_global_scope($user) && $request->has('set_company')) {
            session(['company_id' => (int) $request->query('set_company')]);
            return redirect()->route('employees.allin_overtime');
        }
        ...
        if ($request->isMethod('post')) {
            $data = $request->validate([
                'allin_ot_rate_6_7' => ['required'],
                'allin_ot_rate_7_8' => ['required'],
                'allin_ot_rate_8_9' => ['required'],
                'allin_ot_rate_9_10' => ['required'],
            ]);

            $company->allin_ot_rate_6_7 = (float) ($data['allin_ot_rate_6_7'] ?? 30000);
            $company->allin_ot_rate_7_8 = (float) ($data['allin_ot_rate_7_8'] ?? 35000);
            $company->allin_ot_rate_8_9 = (float) ($data['allin_ot_rate_8_9'] ?? 40000);
            $company->allin_ot_rate_9_10 = (float) ($data['allin_ot_rate_9_10'] ?? 45000);
            $company->save();

            return redirect()->route('employees.allin_overtime', ['ok' => 1]);
        }

        return view('modules.employees.allin_overtime', compact('user', 'companyId', 'companies', 'company'));
    }
    ...
    public function export(Request $request)
    {
        $user = current_user();
        $companyId = current_company_id();
        $employees = $this->buildEmployeesExportQuery($request, $user, $companyId)->get();
    ...
    public function exportExcel(Request $request)
    {
        ...
        if ($tmp === false) {
            abort(500, 'Gagal membuat file export sementara.');
        }
        $xlsxPath = $tmp . '.xlsx';
    ...
    public function exportPdf(Request $request)
    {
        ...
        $html = '<html><head><meta charset="UTF-8"><style>
            ...
            h2{margin:0 0 4px 0; font-size:12px;}
        ...
        $html .= '<h2>Data Karyawan - ' . htmlspecialchars((string) ($company->company_name ?? 'Company'), ENT_QUOTES, 'UTF-8') . '</h2>';
        $html .= '<div class="meta">Total data: ' . (int) $employees->count() . ' | Dicetak: ' . date('d/m/Y H:i') . '</div>';
        $html .= '<table><thead><tr>';
        $headers = [
            'NIK', 'NIK KTP', 'Nama Karyawan', 'Status Aktif', 'Alamat KTP', 'Alamat Domisili',
            'Tempat Lahir', 'Tanggal Lahir', 'Nomor HP', 'Nomor Tlp Saudara/Famili', 'NPWP',
            'Nama Bank', 'Nomor Rekening', 'PTKP Status', 'Status Karyawan', 'Staff / Non Staff',
            'Departement', 'Jabatan', 'Golongan', 'Tanggal Join', 'Habis Kontrak'
        ];
        ...
        foreach ($employees as $e) {
            $cells = [
                $e->nik, $e->nik_ktp, $e->name, $e->active_status, $e->address_ktp, $e->domicile_address,
                $e->place_of_birth, format_date_id($e->date_of_birth), $e->phone, $e->emergency_contact_number, $e->npwp,
                $e->bank_name, $e->bank_account_no, $e->ptkp_status, $e->employment_status, $e->employee_type,
                $e->department, $e->position, $e->grade, format_date_id($e->join_date), format_date_id($e->contract_end),
            ];
            $html .= '<tr>';
        ...
        return response($dompdf->output())
            ...
            ->header('Content-Disposition', 'attachment; filename="employees_export.pdf"');
    }
