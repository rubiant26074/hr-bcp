...
class AttendanceController extends Controller
{
    ...
    private function applyExcludeInactiveEmployeeStatuses($query, string $alias = 'e')
    {
        ...
        return $query
            ->where(function ($q) use ($employmentStatusColumn) {
            ...
            ->where(function ($q) use ($activeStatusColumn) {
            $q->whereNull($activeStatusColumn)
                ->orWhereRaw("TRIM(COALESCE($activeStatusColumn, '')) = ''")
                ->orWhereRaw(
                    "LOWER(TRIM(COALESCE($activeStatusColumn, ''))) NOT IN (?, ?, ?)",
                    [
    ...
    public function faceEnrollNative(Request $request)
    {
        $user = current_user();
        if (!$user) {
            return response()->json(['ok' => false, 'message' => 'Unauthorized'], 401);
        ...
        $data = $request->validate([
            'stable_frames' => ['nullable', 'integer', 'min:0'],
        ]);

        $stableFrames = (int) ($data['stable_frames'] ?? 0);
        if ($stableFrames < 6) {
            return response()->json(['ok' => false, 'message' => 'Deteksi wajah belum stabil untuk pendaftaran.'], 422);
        }

        // Mode transisi native:
        // simpan marker descriptor agar akun dianggap sudah enroll,
        // tanpa mengubah skema database saat ini.
        FaceProfile::updateOrCreate(
            ['user_id' => (int) $user['id']],
            ['descriptor' => [999.01, 999.02, 999.03, 999.04, 999.05, 999.06, 999.07, 999.08, 999.09, (float) $stableFrames]]
        );

        return response()->json(['ok' => true, 'message' => 'Wajah native berhasil didaftarkan.']);
    }
    ...
    public function faceCheckinNative(Request $request)
    {
        $user = current_user();
        if (!$user) {
            return response()->json(['ok' => false, 'message' => 'Unauthorized'], 401);
        ...
        $stableFrames = (int) ($data['stable_frames'] ?? 0);
        if ($stableFrames < 6) {
            return response()->json(['ok' => false, 'message' => 'Deteksi wajah belum stabil.'], 422);
        }
        ...
        // Native mode tetap mensyaratkan user sudah pernah enroll wajah,
        // agar tidak bypass pendaftaran wajah awal.
        $profile = FaceProfile::where('user_id', (int) $user['id'])->first();
        if (!$profile || empty($profile->descriptor)) {
            return response()->json(['ok' => false, 'message' => 'Wajah belum didaftarkan.'], 422);
        }
        ...
        AttendanceService::insertLog([
            ...
            'device_user_id' => (string) ($user['email'] ?? ''),
            'scan_time' => date('Y-m-d H:i:s'),
            'verify_type' => 'mobile_native_' . strtolower($mode),
            'device_id' => 'android_native',
        ]);

        return response()->json([
            'ok' => true,
            'message' => 'Absensi ' . ($mode === self::ATTENDANCE_MODE_IN ? 'masuk' : 'pulang') . ' berhasil (native).',
        ]);
    }
    ...
    private function rememberDeviceUserMapping(int $companyId, ?int $employeeId, ?string $deviceUserId): void
    {
        AttendanceService::upsertDeviceUserEmployeeMap($companyId, $employeeId, $deviceUserId);
    }
    ...
    public function import(Request $request)
    {
        ...
        $messages = [];
        $chunkState = session('attendance_import_chunk');
        if (is_array($chunkState) && !empty($chunkState['path']) && !is_file((string) $chunkState['path'])) {
            session()->forget('attendance_import_chunk');
            $chunkState = null;
        }
        if ($request->isMethod('post')) {
            $action = trim((string) $request->input('action', 'import_now'));
            ...
            if ($action === 'process_chunk') {
                [$chunkMessages, $chunkState] = $this->processCsvChunkImport();
                $messages = array_merge($messages, $chunkMessages);
                return view('modules.attendance.import', compact('user', 'companyId', 'importCompanyId', 'companies', 'messages', 'chunkState'));
            }
            ...
            if (!$request->hasFile('file')) {
                ...
                if (!$file->isValid()) {
                    $messages[] = 'File upload failed.';
                } elseif ($file->getSize() > 20 * 1024 * 1024) {
                    $messages[] = 'File terlalu besar (maks 20MB).';
                } else {
                    ...
                    if (!in_array($ext, ['csv', 'xls', 'xlsx'], true)) {
                        ...
                        if ($action === 'start_chunk') {
                            [$chunkMessages, $chunkState] = $this->startCsvChunkImport($importCompanyId, $file);
                            $messages = array_merge($messages, $chunkMessages);
                            return view('modules.attendance.import', compact('user', 'companyId', 'importCompanyId', 'companies', 'messages', 'chunkState'));
                        }
                        ...
                        if (!empty($missing)) {
                            ...
                            while (($row = fgetcsv($handle, 0, $delimiter, '"', '\\')) !== false) {
                                ...
                                $this->rememberDeviceUserMapping($targetCompanyId, $employeeId, $deviceUserId);
    ...
    private function startCsvChunkImport(int $importCompanyId, $file): array
    {
        $handle = fopen($file->getPathname(), 'r');
        if (!$handle) {
            return [['Gagal membaca file CSV.'], null];
        }
        ...
        if (!empty($missing)) {
            fclose($handle);
            return [['Mode chunk hanya untuk CSV standar. Header kurang: ' . implode(', ', $missing) . '.'], null];
        }

        $offset = ftell($handle);
        fclose($handle);

        $tmpDir = storage_path('app/attendance_chunks');
        if (!is_dir($tmpDir)) {
            File::makeDirectory($tmpDir, 0755, true);
        }
        $token = 'chunk_' . date('Ymd_His') . '_' . bin2hex(random_bytes(4));
        $tmpPath = $tmpDir . DIRECTORY_SEPARATOR . $token . '.csv';
        ...
        session(['attendance_import_chunk' => $chunkState]);
        return [['Import bertahap dimulai. Klik "Lanjut Proses Chunk" sampai selesai.'], $chunkState];
    }

    private function processCsvChunkImport(): array
    {
        $chunkState = session('attendance_import_chunk');
        if (!is_array($chunkState) || empty($chunkState['path']) || !is_file((string) $chunkState['path'])) {
            return [['Tidak ada proses chunk aktif. Upload file lalu pilih "Import Bertahap (Chunk)".'], null];
        }

        $path = (string) $chunkState['path'];
        $delimiter = (string) ($chunkState['delimiter'] ?? ',');
        $map = (array) ($chunkState['map'] ?? []);
        $importCompanyId = (int) ($chunkState['import_company_id'] ?? current_company_id());
        $offset = (int) ($chunkState['offset'] ?? 0);
        $batchSize = max(100, min(2000, (int) ($chunkState['batch_size'] ?? 100)));
        $companyImportStats = (array) ($chunkState['company_stats'] ?? []);
        $unknown = (int) ($chunkState['unknown_count'] ?? 0);
        $insertedBefore = (int) ($chunkState['inserted_count'] ?? 0);

        $handle = fopen($path, 'r');
        if (!$handle) {
            session()->forget('attendance_import_chunk');
            return [['Gagal membuka file chunk. Proses dibatalkan.'], null];
        }
        fseek($handle, $offset);

        $processedRows = 0;
        $insertedInBatch = 0;
        while ($processedRows < $batchSize && ($row = fgetcsv($handle, 0, $delimiter, '"', '\\')) !== false) {
            if (is_array($row)) {
                $row = $this->normalizeMalformedCsvRow($row, $delimiter);
            }
            $processedRows++;
        ...
        $chunkState['offset'] = $newOffset;
        $chunkState['unknown_count'] = $unknown;
        $chunkState['inserted_count'] = $insertedBefore + $insertedInBatch;
        $chunkState['company_stats'] = $companyImportStats;

        if ($eof || $processedRows === 0) {
            $scopeText = $importCompanyId === 0 ? ' (semua entitas)' : '';
            $messages = ['Import chunk selesai' . $scopeText . ': ' . (int) $chunkState['inserted_count'] . ' log. Unknown employee: ' . (int) $chunkState['unknown_count'] . '.'];
            foreach ($this->formatImportCompanySummary($companyImportStats) as $line) {
                $messages[] = $line;
            }
            if (is_file($path)) {
        ...
        session(['attendance_import_chunk' => $chunkState]);
        return [[
            'Chunk diproses: +' . $insertedInBatch . ' log. Total sementara: ' . (int) $chunkState['inserted_count'] . ' log.',
            'Klik "Lanjut Proses Chunk" untuk lanjut batch berikutnya.',
        ], $chunkState];
    }
            ...
            for ($r = $subHeaderRowIdx + 1; $r < count($rows); $r++) {
                ...
                foreach ($colMap as $colIdx) {
                    ...
                    foreach ($times as $t) {
                        ...
                        $deviceUserId = $block['no'] ?: $block['name'];
                        $this->rememberDeviceUserMapping($targetCompanyId, $employeeId, $deviceUserId);
                        AttendanceService::insertLog([
                            'company_id' => $targetCompanyId,
                            'employee_id' => $employeeId,
    ...
    private function normalizedNameTokens(string $value): array
    {
        $norm = $this->normalizeNameMatch($value);
        if ($norm === '') {
            return [];
        ...
        return array_values(array_filter(explode(' ', $norm), static fn ($token) => $token !== ''));
    }

    private function scoreNameTokenPair(string $needleToken, string $candidateToken): float
    {
        ...
        $needleLen = strlen($needleToken);
        $candidateLen = strlen($candidateToken);

        if (
            ($needleLen === 1 || $candidateLen === 1)
            && $needleToken[0] === $candidateToken[0]
        ) {
            return 0.93;
        }

        if (str_starts_with($needleToken, $candidateToken) || str_starts_with($candidateToken, $needleToken)) {
            $minLen = min($needleLen, $candidateLen);
            if ($minLen >= 3) {
                return 0.95;
            }
            if ($minLen === 2) {
        ...
        if (function_exists('metaphone')) {
            $metaNeedle = metaphone($needleToken);
            $metaCandidate = metaphone($candidateToken);
            if ($metaNeedle !== '' && $metaNeedle === $metaCandidate) {
                return 0.84;
            }
        }

        if (soundex($needleToken) === soundex($candidateToken)) {
            return 0.80;
        }
        ...
        $distance = levenshtein($needleToken, $candidateToken);
        $ratio = 1.0 - ($distance / $maxLen);
        if ($ratio >= 0.80) {
            return min(0.79, $ratio);
        }
        if ($ratio >= 0.67 && $maxLen >= 5) {
            return 0.68;
        }
    ...
    private function scoreNameCandidateTokens(array $needleTokens, array $candidateTokens): array
    {
        if (count($needleTokens) === 0 || count($candidateTokens) === 0) {
            return ['score' => 0.0, 'strong_count' => 0, 'last_token_score' => 0.0];
        }
        ...
        $strongCount = 0;
        $lastTokenScore = 0.0;
        $lastIndex = count($needleTokens) - 1;

        foreach ($needleTokens as $idx => $needleToken) {
            $best = 0.0;
            foreach ($candidateTokens as $candidateToken) {
                $score = $this->scoreNameTokenPair($needleToken, $candidateToken);
                if ($score > $best) {
                    $best = $score;
                }
            }
        ...
        $avg = $sum / max(1, count($needleTokens));
        $coverage = $matched / max(1, count($needleTokens));
        $score = ($avg * 0.75) + ($coverage * 0.25);

        if ($lastTokenScore >= 0.95) {
            $score += 0.06;
        } elseif ($lastTokenScore >= 0.88) {
            $score += 0.03;
        }
        ...
        return [
            ...
            'last_token_score' => $lastTokenScore,
        ];
    }
    ...
    private function resolveEmployeeIdByApproximateName(int $companyId, string $employeeName): ?int
    {
        ...
        $needleTokens = $this->normalizedNameTokens($employeeName);

        $query = Employee::select('id', 'name');
        if ($companyId > 0) {
            $query->where('company_id', $companyId);
        }
        // Narrow candidate set first to avoid expensive fuzzy scoring across the whole company.
        if (count($needleTokens) > 0) {
            $lead = $needleTokens[0] ?? '';
            $tail = $needleTokens[count($needleTokens) - 1] ?? '';
            $query->where(function ($q) use ($lead, $tail) {
                if ($lead !== '') {
                    $q->orWhere('name', 'like', '%' . $lead . '%');
                }
                if ($tail !== '' && $tail !== $lead) {
                    $q->orWhere('name', 'like', '%' . $tail . '%');
                }
            });
        } else {
        ...
        foreach ($candidates as $candidate) {
            ...
            $candidateTokens = $this->normalizedNameTokens((string) ($candidate->name ?? ''));

            $distance = levenshtein($needle, $norm);
            $tokenStats = $this->scoreNameCandidateTokens($needleTokens, $candidateTokens);

            $ranked[] = [
                'id' => (int) ($candidate->id ?? 0),
                'token_score' => (float) ($tokenStats['score'] ?? 0.0),
                'strong_count' => (int) ($tokenStats['strong_count'] ?? 0),
                'last_token_score' => (float) ($tokenStats['last_token_score'] ?? 0.0),
        ...
        usort($ranked, static function (array $a, array $b): int {
            if ($a['token_score'] !== $b['token_score']) {
                return $a['token_score'] < $b['token_score'] ? 1 : -1;
            }
            if ($a['strong_count'] !== $b['strong_count']) {
                return $a['strong_count'] < $b['strong_count'] ? 1 : -1;
            }
            if ($a['last_token_score'] !== $b['last_token_score']) {
                return $a['last_token_score'] < $b['last_token_score'] ? 1 : -1;
            }
        ...
        $hasStrongNeedle = count($needleTokens) >= 2;
        $minTokenScore = $hasStrongNeedle ? 0.78 : 0.92;
        $minLastTokenScore = $hasStrongNeedle ? 0.80 : 0.0;

        $isAmbiguous =
            $second !== null
            && $best['token_score'] >= 0.82
            && abs($best['token_score'] - $second['token_score']) < 0.05;

        if (
            $best['id'] > 0
            && !$isAmbiguous
            && $best['token_score'] >= $minTokenScore
            && $best['last_token_score'] >= $minLastTokenScore
        ) {
            return (int) $best['id'];
        }
    ...
    private function resolveEmployeeIdByHistoryAnyCompany(string $deviceUserId): ?int
    {
        $deviceUserId = trim($deviceUserId);
        if ($deviceUserId === '') {
            return null;
        ...
        $candidates = DB::table('attendance_logs as l')
            ->join('employees as e', 'e.id', '=', 'l.employee_id')
            ->where('l.device_user_id', $deviceUserId)
            ->whereNotNull('l.employee_id')
            ->groupBy('l.employee_id')
    ...
    private function resolveEmployeeIdAcrossCompanies(array $idCandidates, string $employeeName): ?int
    {
        foreach ($idCandidates as $candidate) {
            ...
            $matches = Employee::select('id')->where('nik', $candidate)->limit(2)->get();
            if ($matches->count() === 1) {
                return (int) ($matches->first()->id ?? 0) ?: null;
            }
        }
        ...
        foreach ($idCandidates as $candidate) {
            ...
            $fromSuffix = $this->resolveEmployeeIdByNumericNikSuffix(0, $candidate);
            if ($fromSuffix !== null) {
                return $fromSuffix;
            }
        }
        ...
        foreach ($idCandidates as $candidate) {
            ...
            $fromHistory = $this->resolveEmployeeIdByHistoryAnyCompany($candidate);
            if ($fromHistory !== null) {
                return $fromHistory;
            }
        }
        ...
        if ($employeeName !== '') {
            $exactNameMatches = Employee::select('id')
                ->where('name', $employeeName)
                ->limit(2)
                ->get();
            if ($exactNameMatches->count() === 1) {
                return (int) ($exactNameMatches->first()->id ?? 0) ?: null;
            }
        }
    ...
    private function findEmployeeIdByNoOrName(int $companyId, string $employeeNo, string $employeeName, int &$unknown, ?int $preferredCompanyId = null): ?int
    {
        ...
        if ($effectiveCompanyId > 0) {
            $crossCompany = $this->resolveEmployeeIdAcrossCompanies([$employeeNo], $employeeName);
    ...
    private function findEmployeeIdForImport(int $companyId, \Closure $getValue, int &$unknown, ?int $preferredCompanyId = null): ?int
    {
        ...
        if ($effectiveCompanyId > 0) {
            $crossCompany = $this->resolveEmployeeIdAcrossCompanies(
                [$idNumber, $cardNo, $deviceUserId],
                $employeeName
            );
            if ($crossCompany !== null) {
                return $crossCompany;
    ...
    public function daily(Request $request)
    {
        ...
        if ($request->isMethod('post') && $request->input('action') === 'save_daily_verification') {
            ...
            $updatedExcuse = AttendanceService::saveExcuseByCompanyDate(
                ...
                $request->input('is_special_leave_excused', [])
            );
        ...
        if ($request->query('saved_excuse')) {
            $messages[] = 'Verifikasi cuti/sakit/izin khusus tersimpan. Jumlah data yang diperbarui: ' . (int) $request->query('saved_excuse') . '.';
        }
        if ($onlyPresent) {
        ...
        $isNationalHoliday = Holiday::where('company_id', 0)
    ...
    public function monthly(Request $request)
    {
        ...
        $rowsQuery = DB::table('employees as e')
            ->leftJoin('attendance_daily as d', function ($q) use ($startDate, $endDate) {
                $q->on('d.employee_id', '=', 'e.id')
                  ->whereBetween('d.date', [$startDate, $endDate]);
            })
            ->leftJoin('payroll_setting as ps', 'ps.employee_id', '=', 'e.id')
        ...
        $rows = $rowsQuery
            ->groupBy('e.id', 'e.nik', 'e.name', 'e.employment_status', 'e.position', 'ps.absence_mode', 'ps.manual_present_days')
            ->orderBy('e.name')
            ->select(
                'e.nik',
                'e.name',
                'e.employment_status',
                'e.position',
                DB::raw('COUNT(d.date) as hadir'),
                DB::raw('SUM(d.work_hours) as total_hours'),
                DB::raw('SUM(d.overtime_hours) as overtime'),
                DB::raw("LOWER(TRIM(COALESCE(ps.absence_mode, 'auto'))) as absence_mode"),
                DB::raw('COALESCE(ps.manual_present_days, 0) as manual_present_days')
            )
            ->get();

        $companyStandardHours = max(0.0, (float) ($company->work_duration_hours ?? 8));
        foreach ($rows as $r) {
            $isManualAbsence = strtolower(trim((string) ($r->absence_mode ?? 'auto'))) === 'manual';
            if (!$isManualAbsence) {
                continue;
            }

            $manualPresentDays = max(0.0, (float) ($r->manual_present_days ?? 0));
            $r->hadir = $manualPresentDays;

            $employmentStatus = strtoupper(trim((string) ($r->employment_status ?? '')));
            if ($employmentStatus === 'HARIAN') {
                $position = strtoupper(trim((string) ($r->position ?? '')));
                $isSecurity = !str_contains($position, 'KEPALA SECURITY')
                    && !str_contains($position, 'KEPALA SCURITY')
                    && (str_contains($position, 'SECURITY') || str_contains($position, 'SCURITY') || str_contains($position, 'SATPAM'));
                $dailyHours = $isSecurity ? 8.0 : $companyStandardHours;
                $r->total_hours = round($manualPresentDays * $dailyHours, 2);
            }
        }

        if ($onlyPresent) {
            $rows = $rows->filter(static function ($r) {
                return (float) ($r->hadir ?? 0) > 0;
            })->values();
        }
    ...
    public function monthlyEmployee(Request $request)
    {
        ...
        if ($isEmployeeRole && $selfEmployeeId <= 0) {
            $selfEmployeeId = (int) (DB::table('users')
                ->where('id', (int) ($user['id'] ?? 0))
                ->value('employee_id') ?? 0);
        }
        if ($isEmployeeRole && $selfEmployeeId <= 0 && !empty($user['email'])) {
            $selfEmployeeId = (int) (DB::table('users')
        ...
        if ($isEmployeeRole) {
            if ($selfEmployeeId > 0) {
                $employeesQuery->where('id', $selfEmployeeId);
            } else {
        ...
        if ($isEmployeeRole && $selfEmployeeId <= 0) {
            $messages[] = 'Akun Employee belum terhubung ke data karyawan. Hubungi HR untuk mapping akun.';
        }

        if ($request->isMethod('post') && (string) $request->input('action') === 'save_monthly_verification') {
            if ($isEmployeeRole) {
                $messages[] = 'Mode employee hanya dapat melihat rekap absensi (read-only).';
                ...
                $checkedSpecialLeave = array_flip(array_values(array_filter(array_map(static function ($v) {
                    $s = trim((string) $v);
                    return preg_match('/^\d{4}-\d{2}-\d{2}$/', $s) ? $s : '';
                ...
                if (count($validDates) > 0) {
                    $existingRows = DB::table('attendance_daily')
                        ...
                        ->get([
                            ...
                            DB::raw('COALESCE(is_special_leave_excused, 0) as is_special_leave_excused'),
                        ]);
                    $existingMap = [];
                    ...
                    try {
                        foreach ($validDates as $date) {
                            ...
                            if ($hasAttendance) {
                                ...
                                // TA-IL hanya flag validasi izin lembur.
                                // Pertahankan jam lembur existing supaya tidak ter-reset ke 0 setelah checklist.
                                $overtimeHours = round(max(0.0, (float) ($existing->overtime_hours ?? 0)), 2);
                                if ($overtimeHours <= 0) {
                                ...
                                if ($hasSpecialLeaveExcusedColumn) {
                                    ...
                                    $existingSpecial = (int) ($existing->is_special_leave_excused ?? 0);
                                    $requestedSpecial = isset($checkedSpecialLeave[$date]) ? 1 : 0;
                                    if ($existingSpecial === 1 && $requestedSpecial === 0 && !$canUnlockSpecialLeave) {
                                        $requestedSpecial = 1;
                                    }
                                    $updatePayload['is_special_leave_excused'] = $requestedSpecial;
                                }
                            ...
                            $specialLeaveFlag = isset($checkedSpecialLeave[$date]) ? 1 : 0;
                            $existingSpecial = (int) ($existing->is_special_leave_excused ?? 0);
                            if ($existingSpecial === 1 && $specialLeaveFlag === 0 && !$canUnlockSpecialLeave) {
                                $specialLeaveFlag = 1;
                            }
                            ...
                            if ($existing) {
                                ...
                                $updated += DB::table('attendance_daily')
                                    ...
                                    ->where('date', $date)
                                    ->update($updatePayload);
                            } elseif ($leaveFlag === 1 || $sickFlag === 1 || $specialLeaveFlag === 1) {
                                if ($hasSpecialLeaveExcusedColumn) {
        ...
        if ($selectedEmployee) {
            $selectedPosition = strtoupper(trim((string) ($selectedEmployee->position ?? '')));
            $isSelectedSecurity =
                !str_contains($selectedPosition, 'KEPALA SECURITY')
                && !str_contains($selectedPosition, 'KEPALA SCURITY')
                && (
                    str_contains($selectedPosition, 'SECURITY')
                    || str_contains($selectedPosition, 'SCURITY')
                    || str_contains($selectedPosition, 'SATPAM')
                );
            ...
            if ($isSelectedSecurity) {
                $rebuildCursor = $startDate;
                while ($rebuildCursor <= $endDate) {
                    AttendanceService::rebuildDaily($companyId, $rebuildCursor);
                    $rebuildCursor = date('Y-m-d', strtotime($rebuildCursor . ' +1 day'));
                }
            }

            $securityShiftMap = $isSelectedSecurity
                ? AttendanceService::buildSecurityShiftDailyMap($companyId, (int) $selectedEmployee->id, $startDate, $endDate)
                : [];
            ...
            if ($hasSpecialLeaveExcusedColumn) {
                $dailySelect[] = DB::raw('COALESCE(is_special_leave_excused, 0) as is_special_leave_excused');
            } else {
                $dailySelect[] = DB::raw('0 as is_special_leave_excused');
            }
            ...
            if ($isSelectedSecurity) {
                foreach ($securityShiftMap as $shiftDate => $shiftDaily) {
                    $existingFlags = $dailyMap[$shiftDate] ?? null;
                    $dailyMap[$shiftDate] = (object) [
                        'date' => $shiftDate,
                        ...
                        'is_special_leave_excused' => (int) ($existingFlags->is_special_leave_excused ?? 0),
                    ];
                }
            }
            ...
            $cursor = $startDate;
            $startLoopDt = new \DateTime($startDate);
            while ($cursor <= $endDate) {
                $dow = date('D', strtotime($cursor));
                $daily = $dailyMap[$cursor] ?? null;
                $securityShiftCode = null;
                $securityShiftStart = null;
                $securityMissingCheckout = false;
                if ($isSelectedSecurity) {
                    $shiftMeta = $securityShiftMap[$cursor] ?? null;
                    $securityShiftCode = $shiftMeta['shift_code'] ?? null;
                    $securityMissingCheckout = (bool) ($shiftMeta['is_missing_checkout'] ?? false);
                    if (!empty($shiftMeta['shift_start_at'])) {
                        $securityShiftStart = date('H:i:s', strtotime((string) $shiftMeta['shift_start_at']));
                    }
                    if (($shiftMeta['is_off'] ?? false) === true) {
                        $daily = null;
                    }
                }
                ...
                $status = 'Tidak Hadir';
                $isLeaveExcused = (int) ($daily->is_leave_excused ?? 0) === 1;
                $isSickDoctorExcused = (int) ($daily->is_sick_doctor_excused ?? 0) === 1;
                $isSpecialLeaveExcused = (int) ($daily->is_special_leave_excused ?? 0) === 1;
                if ($isSelectedSecurity && $securityShiftCode === 'OFF') {
                    $status = 'OFF';
                } elseif ($isSelectedSecurity && $securityMissingCheckout) {
                    $status = 'Perlu Konfirmasi';
                } elseif ($hasAttendance || $isSickDoctorExcused || $isSpecialLeaveExcused) {
                    $status = 'Hadir';
                    $summary['hadir']++;
                } elseif ($isLeaveExcused) {
                    $status = 'Izin Cuti';
                    $summary['izin']++;
                ...
                $hasChecklistFlag =
                    ((int) ($daily->no_overtime_permit ?? 0) === 1)
                    || ((int) ($daily->is_leave_excused ?? 0) === 1)
                    || ((int) ($daily->is_sick_doctor_excused ?? 0) === 1)
                    || ((int) ($daily->is_special_leave_excused ?? 0) === 1);
                if ($status === 'Perlu Konfirmasi') {
                    $remark = 'Absensi shift belum lengkap, perlu Konfirmasi';
                } elseif ($status === 'Hadir' && $workHours <= 0 && !$hasChecklistFlag) {
                    $remark = 'Gagal Absen perlu Konfirmasi';
                }

                $displayDate = $cursor;
                $displayDayName = $dayLabels[$dow] ?? $dow;
                if (!empty($daily->check_out)) {
                    $outDate = date('Y-m-d', strtotime((string) $daily->check_out));
                    if ($outDate > $cursor) {
                        $outDow = date('D', strtotime($outDate));
                        $displayDate = $cursor . ' - ' . $outDate;
                        $displayDayName = ($dayLabels[$dow] ?? $dow) . '-' . ($dayLabels[$outDow] ?? $outDow);
                    }
                }
                ...
                $rows[] = (object) [
                    ...
                    'is_special_leave_excused' => (int) ($daily->is_special_leave_excused ?? 0),
                    'has_attendance' => $hasAttendance,
                    'status' => $status,
                    'holiday_name' => $remark,
                    'security_shift_code' => $securityShiftCode,
                    'security_shift_start' => $securityShiftStart,
                    'security_missing_checkout' => $securityMissingCheckout,
                ];
    ...
    public function report(Request $request)
    {
        ...
        if (current_user_has_global_scope($user) && $request->has('set_company')) {
            session(['company_id' => (int) $request->query('set_company')]);
            $params = [];
            if ($request->query('period_mode')) {
                $params['period_mode'] = $request->query('period_mode');
            }
            if ($request->query('month')) {
                $params['month'] = $request->query('month');
            }
        ...
        $periodMode = (string) $request->query('period_mode', 'cutoff');
        ...
        $dateFrom = date_input_to_db($dateFromInput);
        $dateTo = date_input_to_db($dateToInput);
        ...
        $rows = $rows
            ...
            ->orderBy('e.name')
            ->select('e.nik', 'e.name', DB::raw('COUNT(DISTINCT d.date) as hadir'),
                DB::raw('SUM(COALESCE(d.overtime_hours, 0)) as overtime'))
            ->get();

        return view('modules.attendance.report', compact(
            'user',
            'companyId',
            'companies',
            'periodMode',
